...
required = {"users", "departments", "department_links", "memberships", "reporting_lines", "role_grants"}
if payload.get("version") != 2 or not required.issubset(payload):
...
trap - EXIT
echo "Export CSRS v2 créé avec un mode 0600; son contenu sensible n'a pas été affiché."
